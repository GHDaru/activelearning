...
- **Correção**: as 6 figuras de `experiments/plots/` (E1/E4/E6×2/ciclo/E0) NÃO
  eram mais `sem-evidencia` — já estavam versionadas e reproduzem byte-a-byte.
  O achado certo é outro: nenhuma das 6 está `\includegraphics`'da no texto
  atual do Cap. 4/5. A única figura de fato no texto hoje
  (`L0tendenciagerallog.png`, Cap. 4) é que ficou `sem-evidencia`: sem
  gerador neste repositório, commitada direto no tesedaru.
- Três números do Cap. 3 sem artefato recuperável: janela de estagnação
  ...
  (1/10/25 por McNemar).
- Decisão do `principal`/autor sobre os sete achados acima.
"""))
